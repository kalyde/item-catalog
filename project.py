...
# JSON Endpoint for Category Menu
...
def CatalogMenu(category_id):
    ...
    if ('username' not in login_session or
        ...
        return render_template(
            'publicmenu.html', category1=category1, category=category,
            items=items, category_id=category_id)
    else:
        return (render_template('menu.html', category1=category1,
        category=category, items=items, category_id=category_id))
...
#            "order to edit.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
            ...
            return redirect(url_for('CatalogMenu', category_id=category_id))
    else:
...
#            "order to delete it.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('Catalog', category=category_id))
    else:
...
#            "</script><body onload='myFunction()'>"
    if request.method == 'POST':
        ...
        return render_template(
            'editmenuitem.html', category_id=category_id, menu_id=menu_id,
            item=editedItem)
